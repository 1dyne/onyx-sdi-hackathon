...
const validation = (() => {
  ...
  function baselineQuality(samples, start, end) {
    ...
    if(fixedAtMax.length) reasons.push(`센서 상한 고정(CH${fixedAtMax.map(ch=>ch+1).join(', CH')}): 변화를 읽을 수 없음`);
    else if(base.some(v=>v>=1000)) warnings.push('센서 상한 근접: 작은 변화(3–5)를 기준으로 기록');
    return {ok:!reasons.length,reasons,warnings,count:n,spanMs:span,maxGapMs:Math.max(...gaps),baseline:base,spread,headroom:base.map(v=>1023-v),method:'fresh-packets-median-v1', limits:{minPackets:10,minSpanMs:2000,maxGapMs:600,maxSpread:35,nearSaturation:1000,hardSaturation:1023,minVisibleDelta:3}};
  }
